...
if [[ $method == *"metaicl" || $method == *"multitask-zero" ]] ; then
    ...
    if [[ ! -f $checkpoint ]] ; then
        python3 -m utils.download --checkpoints --setting $task --method $method
    fi
else
    out_dir="checkpoints/lm"
fi
if [[ $method == "metaicl" ]] ; then
    python3 test.py --task $task --k 16 --split test --seed $seed --use_demonstrations \
...
if [[ $method == "multitask-zero" ]] ; then
    python3 test.py --task $task --split test \
    --test_batch_size $bs --method direct --checkpoint $checkpoint --out_dir $out_dir
fi
...
if [[ $method == "zero" ]] ; then
    python3 test.py --task $task --k 16 --split test \
    --test_batch_size $bs --method direct --out_dir $out_dir --do_zeroshot
fi
if [[ $method == "pmi-zero" ]] ; then
    python3 test.py --task $task --k 16 --split test --is_null \
    --test_batch_size $bs --method direct --out_dir $out_dir --do_zeroshot
    python3 test.py --task $task --k 16 --split test --use_calibration \
    --test_batch_size $bs --method direct --out_dir $out_dir --do_zeroshot
fi
...
if [[ $method == "ic" ]] ; then
    python3 test.py --task $task --k 16 --seed $seed --split test \
    --test_batch_size $bs --method direct --use_demonstrations --out_dir $out_dir --do_zeroshot
fi
if [[ $method == "pmi-ic" ]] ; then
    python3 test.py --task $task --k 16 --seed $seed --split test --is_null \
    --test_batch_size $bs --method direct --use_demonstrations --out_dir $out_dir --do_zeroshot
    python3 test.py --task $task --k 16 --seed $seed --split test --use_calibration \
    --test_batch_size $bs --method direct --use_demonstrations --out_dir $out_dir --do_zeroshot
fi
